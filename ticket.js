...
function parseCategories(input) {
  // Format: "Support|Questions, Signalement|Report, Partenariat"
  // - label est requis
  // - description facultative après "|"
  // Limite Discord: 25 options max
...
function renderTranscript(channel, messages) {
  ...
  const lines = messages.map((m) => {
    const author = m.author
      ? `${m.author.username}${m.author.discriminator && m.author.discriminator !== "0" ? `#${m.author.discriminator}` : ""} (${m.author.id})`
      : "Unknown";
    ...
    const embeds =
      m.embeds && m.embeds.length
        ? `\n[Embeds]\n${m.embeds
            .map((e) => {
              const t = e?.title ? `title="${String(e.title).slice(0, 200)}"` : "";
              const d = e?.description
                ? `desc="${String(e.description).slice(0, 200)}"`
                : "";
              return `- embed ${[t, d].filter(Boolean).join(" ")}`.trim();
            })
            .join("\n")}`
        : "";
...
function createTicketsService({ pool, config }) {
    ...
    .addSubcommand((sub) =>
      sub
        ...
        .addRoleOption((opt) =>
          opt
            .setName("staff_role")
            .setDescription("Rôle staff qui voit/traite les tickets (optionnel)")
            .setRequired(false)
        )
        .addChannelOption((opt) =>
          opt
            .setName("admin_feedback_channel")
            .setDescription("Salon admin-only qui reçoit les feedbacks ⭐")
            .setRequired(false)
            .addChannelTypes(
              ChannelType.GuildText,
              ChannelType.GuildAnnouncement
        ...
        .addChannelOption((opt) =>
          opt
            ...
            .setDescription("Salon admin-only qui reçoit les transcripts (optionnel)")
            .setRequired(false)
            .addChannelTypes(
              ChannelType.GuildText,
              ChannelType.GuildAnnouncement
        ...
        .addBooleanOption((opt) =>
          opt
            ...
            .setDescription("Claim exclusif (optionnel)")
            .setRequired(false)
        )
        .addBooleanOption((opt) =>
          opt
            .setName("delete_on_close")
            .setDescription("Supprimer auto le ticket après fermeture (optionnel)")
            .setRequired(false)
        )
    );
  ...
  function isStaff(interaction, settings) {
    ...
    return Boolean(member?.roles?.cache?.has?.(settings.staff_role_id));
  }
  ...
  async function doClaim(interaction, ticketId) {
    ...
    if (!isStaff(interaction, settings)) {
      await interaction.reply({
        content: "⛔ Staff/Admin uniquement.",
        flags: MessageFlags.Ephemeral,
      });
      return true;
    }
    ...
    if (ticket.claimed_by && ticket.claimed_by !== interaction.user.id) {
      await interaction.reply({
        content: `⚠️ Déjà claim par <@${ticket.claimed_by}>.`,
        flags: MessageFlags.Ephemeral,
      });
      return true;
    }
    ...
    // Optionnel: claim exclusif (on coupe l'envoi au staff_role, mais on garde view)
  ...
  async function doClose(interaction, client, ticketId) {
    ...
    if (interaction.channel && interaction.channel.isTextBased?.()) {
      ...
      // hard cap ~7MB to avoid upload fail
      const maxBytes = 7 * 1024 * 1024;
      const buf = Buffer.from(content, "utf8");
      if (buf.length > maxBytes) {
        content =
          content.slice(0, Math.floor((maxBytes / buf.length) * content.length)) +
          `\n\n[TRUNCATED] Transcript trop long, coupé pour rester sous la limite d’upload.\n`;
      }
    ...
    // Feedback request (DM + fallback in ticket)
    await requestFeedback(interaction, client, ticketId, ticket);

    // Delete on close option (after a short delay)
  ...
  async function requestFeedback(interaction, client, ticketId, ticketRow) {
    ...
    // Fallback in the ticket channel (visible only to people who can see the ticket)
    if (interaction.channel && interaction.channel.isTextBased?.()) {
  ...
  async function doDelete(interaction, client, ticketId) {
    ...
    // If channel got deleted, interaction might fail to reply (depends context), so best effort:
    if (interaction.isRepliable?.()) {
      await interaction
        .reply({ content: "✅ Ticket supprimé.", flags: MessageFlags.Ephemeral })
        .catch(() => {});
    }
    return true;
  }
  ...
  async function doTranscript(interaction, client, ticketId) {
    ...
    // Pull transcript from DB if exists, else generate now if channel exists
    let content = null;
  ...
  async function doRate(interaction, client, ticketId, rating) {
    ...
    // Save rating immediately (comment can come later)
    await pool.query(
    ...
    // Send to admin-only feedback channel (or edit existing)
    const guild = await client.guilds.fetch(ticket.guild_id).catch(() => null);
    if (guild) {
      const adminFeedback = await fetchAdminChannel(guild, settings, "feedback");
      if (adminFeedback) {
        const fbRes = await pool.query(
          `SELECT log_channel_id, log_message_id, comment, created_at
           FROM ticket_feedback WHERE ticket_id=$1 LIMIT 1`,
          [ticketId]
        );
        ...
        // try edit existing log message
        if (fb.log_channel_id && fb.log_message_id) {
          const ch = await guild.channels.fetch(fb.log_channel_id).catch(() => null);
          if (ch && ch.isTextBased?.()) {
            const msg = await ch.messages.fetch(fb.log_message_id).catch(() => null);
            if (msg) {
              await msg.edit({ embeds: [embed] }).catch(() => {});
            } else {
              const sent = await adminFeedback
                .send({ embeds: [embed] })
                .catch(() => null);
    ...
    // Optional comment modal (required=false)
    ...
    await interaction.showModal(modal).catch(async () => {
      // If modal fails, at least ack
      await interaction
        .reply({ content: `Merci ! Note reçue : ${buildStars(r)}`, flags: MessageFlags.Ephemeral })
        .catch(() => {});
    });
  ...
  async function doComment(interaction, client, ticketId, rating) {
    ...
    await pool.query(
      `UPDATE ticket_feedback SET comment=$2, rating=$3, created_at=NOW()
       WHERE ticket_id=$1`,
      [ticketId, comment || null, r]
    );

    // Refresh admin log (edit if exists)
    const settings = await getSettings(ticket.guild_id);
    const guild = await client.guilds.fetch(ticket.guild_id).catch(() => null);
    if (guild) {
      const adminFeedback = await fetchAdminChannel(guild, settings, "feedback");
  ...
  async function doStats(interaction) {
    ...
    const topHandlers = await pool.query(
      `SELECT claimed_by, COUNT(*)::int AS handled, AVG(f.rating)::float AS avg
       FROM tickets t
       JOIN ticket_feedback f ON f.ticket_id=t.ticket_id
       WHERE t.guild_id=$1
         AND t.closed_at >= NOW() - ($2 || ' days')::interval
    ...
    if (topHandlers.rows.length) {
      const lines = topHandlers.rows
        .map((r, i) => {
          const a = r.avg ? Number(r.avg).toFixed(2) : "N/A";
          return `**${i + 1}.** <@${r.claimed_by}> — ${r.handled} • ${a}/5`;
        })
        .join("\n");
      embed.addFields({ name: "Top handlers (avec feedback)", value: lines });
    }
  ...
  async function handleInteraction(interaction, client) {
    ...
    if (interaction.isButton()) {
      ...
      if (action === "claim") {
        const ticketId = parts[2];
        return await doClaim(interaction, ticketId);
      }

      if (action === "close") {
        const ticketId = parts[2];
        return await doClose(interaction, client, ticketId);
      }

      if (action === "delete") {
        const ticketId = parts[2];
        return await doDelete(interaction, client, ticketId);
      }

      if (action === "transcript") {
        const ticketId = parts[2];
        return await doTranscript(interaction, client, ticketId);
      }

      if (action === "rate") {
        // ticket:rate:<ticketId>:<1..5>
        const ticketId = parts[2];
        const rating = parts[3];
        return await doRate(interaction, client, ticketId, rating);
      }
    ...
    if (name === "ticket-panel") return await createPanel(interaction);
        ...
        const embed = new EmbedBuilder()
          ...
          .addFields(
            ...
            },
            {
              ...
              value: s.admin_feedback_channel_id
                ? `<#${s.admin_feedback_channel_id}>`
                : "—",
              inline: true,
            },
            {
            ...
            {
              name: "Max open/user",
              value: String(s.max_open_per_user),
              inline: true,
            },
            {
              name: "Cooldown",
              value: `${Math.floor(s.cooldown_seconds / 60)} min`,
              inline: true,
            },
            {
              name: "Claim exclusif",
              value: s.claim_exclusive ? "✅" : "❌",
              inline: true,
            },
            {
              name: "Delete on close",
              value: s.delete_on_close ? "✅" : "❌",
              inline: true,
            }
          )
          .setTimestamp();
    ...
    if (name === "ticket-stats") return await doStats(interaction);
