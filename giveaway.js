...
function buildGiveawayEmbed({ prize, hostId, winnerCount, endAt, status, participantsCount, requirements, winners }) {
  ...
  const eb = new EmbedBuilder()
    ...
    .addFields(
      ...
      { name: "⏳ Fin", value: endAt ? `<t:${Math.floor(new Date(endAt).getTime() / 1000)}:R>` : "?", inline: true },
  ...
  if (ended) {
    ...
    eb.addFields({
      name: "✅ Résultat",
      value: win.length ? win.map((id) => `<@${id}>`).join(", ") : "Aucun gagnant (pas assez de participants éligibles).",
      inline: false,
    });
  }

  if (cancelled) {
    eb.addFields({ name: "ℹ️ Info", value: "Ce giveaway a été annulé par le staff.", inline: false });
  }
...
function mustBeStaff(interaction) {
  return interaction.memberPermissions && interaction.memberPermissions.has(PermissionsBitField.Flags.ManageGuild);
}
...
function createGiveawayService({ pool, config }) {
    ...
    new SlashCommandBuilder()
      ...
      .addSubcommand((sc) =>
        sc
          ...
          .addStringOption((opt) => opt.setName("prize").setDescription("Lot à gagner").setRequired(true).setMaxLength(200))
          .addStringOption((opt) =>
            opt.setName("duration").setDescription("Durée: ex 30m, 2h, 1d, 1h30m").setRequired(true).setMaxLength(32)
          )
          .addIntegerOption((opt) => opt.setName("winners").setDescription("Nombre de gagnants (défaut: 1)").setRequired(false).setMinValue(1).setMaxValue(20))
          ...
          .addStringOption((opt) =>
            opt.setName("required_roles").setDescription("Rôle(s) requis: mentions ou IDs, séparés par espaces").setRequired(false).setMaxLength(600)
          )
          .addStringOption((opt) =>
            opt.setName("forbidden_roles").setDescription("Rôle(s) interdits: mentions ou IDs, séparés par espaces").setRequired(false).setMaxLength(600)
          )
          .addIntegerOption((opt) =>
            opt.setName("min_account_age_days").setDescription("Âge minimum du compte en jours (0 = désactivé)").setRequired(false).setMinValue(0).setMaxValue(3650)
          )
          .addIntegerOption((opt) =>
            opt.setName("min_vouches").setDescription("Vouches minimum (0 = désactivé)").setRequired(false).setMinValue(0).setMaxValue(100000)
          )
          .addBooleanOption((opt) => opt.setName("mention_everyone").setDescription("@everyone au lancement (défaut: non)").setRequired(false))
      )
      .addSubcommand((sc) =>
        sc
          .setName("end")
          .setDescription("MOD: Terminer un giveaway maintenant")
          .addStringOption((opt) => opt.setName("message_id").setDescription("ID du message giveaway (dans le salon)").setRequired(true).setMaxLength(32))
      )
      .addSubcommand((sc) =>
        sc
          .setName("cancel")
      ...
      .addSubcommand((sc) =>
        sc
          ...
          .addStringOption((opt) => opt.setName("message_id").setDescription("ID du message giveaway").setRequired(true).setMaxLength(32))
          .addIntegerOption((opt) => opt.setName("winners").setDescription("Nombre de nouveaux gagnants (défaut: même nombre)").setRequired(false).setMinValue(1).setMaxValue(20))
      )
        ...
        grp
          ...
          .addSubcommand((sc) =>
            sc
              ...
              .addStringOption((opt) => opt.setName("roles").setDescription("Pour required/forbidden: mentions ou IDs").setRequired(false).setMaxLength(600))
              .addIntegerOption((opt) => opt.setName("number").setDescription("Pour min_account_age_days / min_vouches").setRequired(false).setMinValue(0).setMaxValue(100000))
          )
          .addSubcommand((sc) =>
            sc
              .setName("clear")
  ...
  async function upsertRequirements(giveawayId, patch) {
    ...
    await pool.query(`UPDATE giveaways SET requirements=$2::jsonb WHERE giveaway_id=$1`, [giveawayId, JSON.stringify(merged)]);
    return merged;
  }
  ...
  async function rerollGiveaway(client, giveawayId, overrideWinnerCount) {
    ...
    if (newWinners.length) {
      await channel
        .send(`🎲 Reroll : nouveaux gagnants ${newWinners.map((id) => `<@${id}>`).join(", ")} (lot: **${updated.prize}**).`)
        .catch(() => {});
    } else {
  ...
  async function handleJoinLeave(interaction, client, action) {
    ...
    if (action === "join") {
      ...
      await pool.query(`DELETE FROM giveaway_entries WHERE giveaway_id=$1 AND user_id=$2`, [gRow.giveaway_id, interaction.user.id]);
    ...
    try {
      ...
      if (interaction?.isRepliable?.()) {
        if (!interaction.deferred && !interaction.replied) {
          await interaction.reply({ content: "⚠️ Erreur interne (voir logs).", flags: MessageFlags.Ephemeral }).catch(() => {});
    ...
    setInterval(async () => {
      try {
        ...
        for (const row of res.rows) {
          await finalizeGiveaway(client, String(row.giveaway_id)).catch((e) => console.error("finalizeGiveaway:", e));
        }
      } catch (e) {
